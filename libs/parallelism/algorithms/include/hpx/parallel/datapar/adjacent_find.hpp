...
namespace hpx { namespace parallel { inline namespace v1 { namespace detail {
    ...
    struct datapar_adjacent_find
    {
            ...
            InIter first, Sent_ last, PredProj&& pred_projected)
        {
            ...
            std::cout << "tag_dispatch of adj_diff simd" << std::endl;

            auto ret = util::loop_idx_n<ExPolicy>(0, zip_iter, count - 1,
                tok, [&offset, &pred_projected, &tok](auto const& t, std::size_t i) {
                    using hpx::get;
        ...
        {
            std::cout << "tag_dispatch of adj_diff simdpar" << std::endl;
    ...
    template <typename ExPolicy, typename InIter, typename Sent_, typename PredProj,
    HPX_CONCEPT_REQUIRES_(
        hpx::is_vectorpack_execution_policy<ExPolicy>::value &&
        hpx::parallel::util::detail::iterator_datapar_compatible<InIter>::value
    )>
    inline InIter tag_dispatch(sequential_adjacent_find_t<ExPolicy>, 
        InIter first, Sent_ last, PredProj&& pred_projected)
    {
    ...
    template <typename ExPolicy, typename ZipIter, typename Token, typename PredProj,
    HPX_CONCEPT_REQUIRES_(
        hpx::is_vectorpack_execution_policy<ExPolicy>::value &&
        hpx::parallel::util::detail::iterator_datapar_compatible<ZipIter>::value
    )>
    inline void tag_dispatch(sequential_adjacent_find_t<ExPolicy>,
        std::size_t base_idx, ZipIter part_begin, std::size_t part_count,
        Token& tok, PredProj&& pred_projected)
    {
        return datapar_adjacent_find<ExPolicy>::call(base_idx, part_begin, part_count, tok,
