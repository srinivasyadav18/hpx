...
void adjacent_difference_test()
{
    ...
    test_adjacent_difference(simdpar);

    test_adjacent_difference_async(simd(task));
    test_adjacent_difference_async(simdpar(task));
}
...
void test_adjacent_difference_exception()
{
    ...
    test_adjacent_difference_exception(simdpar, IteratorTag());

    test_adjacent_difference_exception_async(simd(task), IteratorTag());
    test_adjacent_difference_exception_async(simdpar(task), IteratorTag());
}
...
void test_adjacent_difference_bad_alloc()
{
    ...
    test_adjacent_difference_bad_alloc(simdpar, IteratorTag());

    test_adjacent_difference_bad_alloc_async(simd(task), IteratorTag());
    test_adjacent_difference_bad_alloc_async(simdpar(task), IteratorTag());
}
